namespace NetRemapper
{
    public class Mappings(MappingsFormat format)
    {
        ...
        public List<TypeDefinitionEntry> TypeDefinitions { get; } = [];
        ...
        {
            return !Namespaces.Contains(ns) ? null :
                TypeDefinitions.Find(type => type.Names[ns] == name);
        }
        ...
        {
            return !Namespaces.Contains(ns) ? null :
                TypeDefinitions.SelectMany(type => type.MethodDefinitions).FirstOrDefault(method => method.Names[ns] == name);
        }
        ...
        {
            return GetType(type, typeNs)?.MethodDefinitions.Find(method => method.Names[ns] == name);
        }
        ...
        {
            return !Namespaces.Contains(ns) ? null :
                TypeDefinitions.SelectMany(type => type.FieldDefinitions)
        ...
        public FieldDefinitionEntry? GetField(string name, string ns, string type, string typeNs)
        {
            ...
            return GetType(type, typeNs)?.FieldDefinitions.Find(field => field.Names[ns] == name);
        }
        ...
        {
            return GetType(type, typeNs)?.PropertyDefinitions.Find(method => method.Names[ns] == name);
        }
    }
    ...
    public class GenericMappingEntry
    {
        ...
        protected GenericMappingEntry(string[] namespaces, string[] names)
        {
